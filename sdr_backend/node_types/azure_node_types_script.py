...
def get_node_type(file_name):
    """
    Extracts and normalizes the node type from an SVG file name.
    Example: '00001-icon-service-Monitor.svg' -> 'azure_monitor'
    
    Args:
        file_name (str): The name of the SVG file.
    Returns:
        str or None: The normalized node type with "azure_" prefix, or None if invalid.
    """
...
def generate_description(node_type):
    ...
    if node_type in descriptions:
        ...
        readable_name = node_type[6:].replace('_', ' ').title()
...
def process_files_and_generate_json():
    ...
    for file_name in svg_files:
        ...
        if node_type:
            ...
            print(f"Could not parse node type from '{file_name}'. Expected format: '00001-icon-service-Monitor.svg'")

    output_data = {"azure": node_types}
    output_file = 'azure_node_types.json'
