...
_BOM_RX = "utf-8-sig"               # to strip BOM safely
_NON_ALNUM = re.compile(r"[^a-z0-9]+")

def slugify(txt: str) -> str:
    txt = unicodedata.normalize("NFKD", txt).encode("ascii", "ignore").decode("ascii")
    return _NON_ALNUM.sub("-", txt.lower()).strip("-")
...
# ────────────────────────────────────────────────────────────
# Iconify “logos” collections
# ────────────────────────────────────────────────────────────
ICONIFY_COLLECTIONS = ["carbon"]

def fetch_iconify_json(prefix: str) -> Dict:
    url = f"https://raw.githubusercontent.com/iconify/icon-sets/master/json/{prefix}.json"
    resp = requests.get(url, timeout=60)
    text = resp.content.decode(_BOM_RX)
    return json.loads(text)

def iter_iconify() -> Iterable[Dict]:
    """
    Yield rows ready for upsert into tech_taxonomy from the chosen Iconify collections.
    """
    for col in ICONIFY_COLLECTIONS:
        print(f"→ downloading {col}.json")
        try:
            data = fetch_iconify_json(col)
        except Exception as exc:
            print(f"⚠️  failed {col}: {exc}")
            continue

        prefix = data.get("prefix", col)
        icons  = data.get("icons", {})

        provider = "iconify repo"

        for raw_name, meta in icons.items():
            # raw_name = e.g. "aws-lambda", "redis"
            base_slug = slugify(raw_name)
            token     = base_slug

            # Display name: try meta["name"] else humanize raw_name
            disp = meta.get("name")
            if not disp:
                disp = first_cap(base_slug.split("-"))

            kind, sub = guess_kind_subkind(base_slug)  # your heuristic
            ...
            yield {
                ...
                "subkind":      sub,
                "provider":     provider,
                "iconify_id":   f"{prefix}:{raw_name}",
                "aliases":      [disp, raw_name, base_slug],
                "source":       col,
            }
# ────────────────────────────────────────────────────────────
