...
// Import layer utilities from the new file
import { createLayerContainers } from '../components/AI/utils/layerUtils';
...
const ModelWithAI = () => {
  ...
  const [justCompletedAnalysis, setJustCompletedAnalysis] = useState(false);
  ...
  // FOCUSED FIX: Update the projectService.saveProject call
const saveCurrentState = useCallback(
  debounce(async () => {
    ...
    // Get the diagram state (keep your existing code)
    const currentDiagramState = { 
      nodes: nodesRef.current, 
      edges: edgesRef.current 
    };
    
    // FIX FOR PYTHON BACKEND ERROR:
    // Convert projectId to a string '0' if it's null/undefined
    // This prevents the Python "NoneType + int" error
    const safeProjectId = projectId || '0';
    
    try {
      console.log(`Saving project with ID: ${safeProjectId}`);
      
      // Use the safe project ID here
      const result = await projectService.saveProject(
        sessionIdRef.current, 
        currentDiagramState, 
        safeProjectId  // This is the key change
      );
  ...
  // NEW: Safe save wrapper function that provides additional safety
  const safeSave = async () => {
    try {
      if (!sessionId) {
        console.warn("Cannot save - missing session ID");
        toast({
          title: "Cannot Save",
          description: "Session information is missing. Please refresh the page.",
          variant: "destructive",
        });
        return false;
      }
      
      // Verify projectId before saving
      if (projectId === null || projectId === undefined) {
        console.warn("Detected null projectId, using fallback value");
        // We can't modify the backend, so we'll use a temporary local value
        // just for this save operation
        const tempProjectId = '0'; // Temporary ID just for this save
        
        // Create a new save function with the temp ID
        const tempSaveCurrentState = async () => {
          try {
            console.log(`Using temporary projectId '${tempProjectId}' for save operation`);
            const currentDiagramState = { 
              nodes: Array.isArray(nodesRef.current) ? nodesRef.current : [],
            ...
            console.error("Error in temporary save:", err);
            return false;
          }
        };
        
        return await tempSaveCurrentState();
      }
      
      // Normal save with valid projectId
      return await saveCurrentState();
    } catch (error) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (sessionIdRef.current) {
        ...
        // Use safeSave instead of saveCurrentState
        safeSave().catch(err => {
  ...
  // Run threat analysis function - UPDATED TO USE STATE APPROACH
  const handleRunThreatAnalysis = useCallback(async () => {
    try {
      setRunningThreatAnalysis(true);
      setJustCompletedAnalysis(false);
      ...
      // Use the state variable for projectId instead of extracting from URL
      if (!projectId) {
      ...
      if (response && response.threats) {
        ...
        // Mark that we just completed analysis
        setJustCompletedAnalysis(true);

        // Reset this flag after a delay
        setTimeout(() => {
          setJustCompletedAnalysis(false);
        }, 2000);
      }
  ...
  }, [nodes, edges, toast, projectId, sessionId]);
  ...
  // UPDATED: Save function to use safeSave
  const save = async () => {
    if (!sessionId) {
      toast({
        title: "Cannot Save",
    ...
    try {
      ...
      const result = await safeSave(); // Use safeSave instead of saveCurrentState

      if (result) {
        toast({
          title: "Project Saved",
          description: `Project ${projectId || 'unnamed'} saved successfully`,
          variant: "default",
        });
      } else {
    ...
    try {
      ...
      if (isDFDGenerationResponse(response)) {
        ...
        try {
          ...
          const threatModelResponse = await generateThreatModel(safeProjectId, request, toast);
  ...
  useEffect(() => {
    // Check if the project is loaded and we have nodes but no threats
    if (projectLoaded && nodes.length > 0 && (!threats || threats.length === 0) && !runningThreatAnalysis) {
      console.log('No threats found after project load, running automatic threat analysis');
  ...
  }, [projectLoaded, nodes.length, threats, runningThreatAnalysis, handleRunThreatAnalysis]);
  ...
  const createAIFlowDiagramProps = useMemo(() => {
    return {
      ...
      onRunThreatAnalysis: handleRunThreatAnalysis, // Pass the handler
      selectedThreat, // Pass selected threat
      selectedNode, // Pass selected node
    };
  }, [
    ...
    threats,handleRunThreatAnalysis,
    selectedThreat,
    selectedNode, // Add threats as dependency
  ]);
...
// import { debounce } from 'lodash';
// // Import layer utilities from the new file
// import { determineNodeLayer, getLayerStyle, createLayerContainers } from '../components/AI/utils/layerUtils';
