...
// Import the ThreatBadges component
import ThreatBadges from './ThreatBadges';

// Extend NodeProps to include threats and activeSeverityFilter
interface CustomNodeExtendedProps extends NodeProps {
  data: CustomNodeData & {
    threats?: ThreatItem[];
...
const CustomNode = ({ 
  ...
  data,
  selected,
}: CustomNodeExtendedProps) => {
  // Extract values with fallbacks
  const label = data?.label || 'Node';
  const description = data?.description || '';
  const nodeType = data?.nodeType || 'default';
  const iconRenderer = data?.iconRenderer;
  const threats = data?.threats || [];
  const activeSeverityFilter = data?.activeSeverityFilter || 'ALL'; // Extract the active filter
  
  // Access connection flags safely, defaulting to false if undefined
  const hasSourceConnection = data?.hasSourceConnection ?? false;
  const hasTargetConnection = data?.hasTargetConnection ?? false;

  const handleEdit = (nodeId: string) => {
    if (data?.onEdit) {
      data.onEdit(nodeId, label);
    }
  };
  ...
  return (
    <>
      ...
      <div className={`node-fade-in custom-node w-16 h-16 relative`}>
        ...
        {/* Add threat badges to node with active severity filter */}
        <ThreatBadges 
          nodeId={id} 
          threats={threats} 
...
// // Extend NodeProps to include threats and active severity filter
// interface CustomNodeExtendedProps extends NodeProps {
//   data: CustomNodeData & {
//     threats?: ThreatItem[];
//     activeSeverityFilter?: string | null;
...
//   const iconRenderer = data?.iconRenderer;
//   const threats = data?.threats || [];
//   const activeSeverityFilter = data?.activeSeverityFilter || 'ALL';
