    ...
    >
      <div className={`px-2 py-1 flex items-start ${getHeaderGradientStyle(severity)}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="group threat-description-wrapper">
            <p className="font-medium line-clamp-2 text-gray-900 text-xs mb-0.5 font-sans">
              {threat.description}
            </p>
            
            {/* Improved tooltip/hover card that's guaranteed to be visible */}
            <div className="threat-hover-card">
              <div className="font-medium text-[12px] mb-3 text-gray-900">{threat.description}</div>
              ...
              <div className="grid grid-cols-1 gap-2 mb-2">
                <div className="flex items-center">
                  <div className={`
                    ...
                    ${severity.toUpperCase() === 'HIGH' ? 'bg-red-500' : 
                      severity.toUpperCase() === 'MEDIUM' ? 'bg-amber-500' : 'bg-blue-500'}
                  `}></div>
                  <span className="font-semibold text-[11px] text-gray-700">Severity:</span>{' '}
                  <span className={`ml-1 text-[11px] font-bold
                    ${severity.toUpperCase() === 'HIGH' ? 'text-red-700' : 
                      severity.toUpperCase() === 'MEDIUM' ? 'text-amber-700' : 'text-blue-700'}
                  `}>
              ...
              <div className="grid grid-cols-2 gap-3 text-[10px]">
                {threat.properties?.attack_vector && (
                  <div>
                    <span className="font-semibold text-gray-700 block mb-1">Attack Vector:</span>
                    <span className="text-gray-800 bg-blue-50 px-1.5 py-1 rounded block border border-blue-100">{threat.properties.attack_vector as string}</span>
                  </div>
                )}
                
                {threat.properties?.threat_type && (
                  <div>
                    <span className="font-semibold text-gray-700 block mb-1">Type:</span>
                    <span className="text-gray-800 bg-purple-50 px-1.5 py-1 rounded block border border-purple-100">{threat.properties.threat_type as string}</span>
                  </div>
                )}
                
                {threat.properties?.impact && (
                  <div>
                    <span className="font-semibold text-gray-700 block mb-1">Impact:</span>
                    <span className="text-gray-800 bg-red-50 px-1.5 py-1 rounded block border border-red-100">{threat.properties.impact as string}</span>
                  </div>
                )}
                
                {mitigationText && (
                  <div>
                    <span className="font-semibold text-gray-700 block mb-1">Mitigation:</span>
                    <span className="text-gray-800 bg-green-50 px-1.5 py-1 rounded block border border-green-100">{mitigationText.length > 30 ? `${mitigationText.substring(0, 30)}...` : mitigationText}</span>
                  </div>
                )}
              </div>
            </div>
