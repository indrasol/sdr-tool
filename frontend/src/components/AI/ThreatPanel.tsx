...
interface ThreatPanelProps {
  ...
  // Add the new prop for notifying about node threat counts and active filter
  onNodeThreatCountsChange?: (nodeThreatCounts: Record<string, { high: number, medium: number, low: number, total: number }>, activeFilter?: string) => void;
  forceExpanded?: boolean;
  nodes?: Node[]; // Add nodes array to access node labels
}
...
const ThreatCard: React.FC<{ 
  ...
  isSelected?: boolean;
  onSelect?: () => void;
  nodes?: Node[]; // Add nodes to props
}> = ({ threat, severity, isSelected = false, onSelect = () => {}, nodes = [] }) => {
  ...
  // Create state for tooltip position and visibility
  const [tooltipVisible, setTooltipVisible] = useState(false);
  const [tooltipPosition, setTooltipPosition] = useState({ top: 0, left: 0 });
  
  // Handle mouse enter event
  const handleMouseEnter = (e: React.MouseEvent) => {
    // Get dimensions and position of the event target
    const target = e.currentTarget;
    const rect = target.getBoundingClientRect();
    
    setTooltipPosition({
      // Position tooltip above the description (15px above)
      top: rect.top - 5, 
    ...
    >
      <div className={`px-2 py-1 flex items-start ${getHeaderGradientStyle(severity)}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Simple hover description */}
          <div className="relative">
            <p 
              className="font-medium line-clamp-2 text-gray-900 text-xs mb-0.5 font-sans cursor-default"
      ...
      <div className="px-2 py-1 border-t border-gray-100 bg-white">
        ...
        {/* Target Elements - Now with node names instead of IDs */}
        {targetElements.length > 0 && (
          <div className="mt-2">
            ...
            <div className="flex flex-wrap gap-1">
              {targetElements.map((elementId, idx) => {
                const nodeName = getNodeName(elementId);
                const displayText = nodeName !== elementId ? 
                  `${nodeName}` : 
                  elementId;
                ...
                return (
                  <span 
                    key={idx} 
                    className="text-[9px] bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded border border-blue-100" 
                    style={{ fontFamily: 'Geneva, sans-serif', fontWeight: 'bold' }}
                    title={elementId}  // Show ID on hover
                  >
                    {displayText}
                  </span>
                );
      ...
      {/* Global tooltip - Light rectangular style */}
      {tooltipVisible && (
        <div 
          className="fixed z-[9999] bg-white border border-gray-300 text-gray-800 rounded-md px-4 py-3 shadow-lg text-xs"
          style={{
        ...
        >
          <div className="font-medium mb-2 text-gray-900 border-b border-gray-200 pb-1">Threat Description:</div>
          <div className="text-gray-700">{threat.description}</div>
          
          {/* Triangle pointer */}
          <div 
            className="w-2 h-2 bg-white transform rotate-45 absolute border-b border-r border-gray-300"
            style={{
              bottom: '-4px',
              left: '50%',
              marginLeft: '-4px'
...
const ThreatPanel: React.FC<ThreatPanelProps> = ({
  ...
  selectedNode,
  onRunThreatAnalysis,
  onNodeThreatCountsChange,
  forceExpanded = false,
  nodes = [] // Default to empty array
}) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [severityFilter, setSeverityFilter] = useState<string | null>('ALL');
  const [isMinimized, setIsMinimized] = useState(!forceExpanded);
  // Add state for tracking node threat counts
  ...
  // Add useEffect to calculate node threat counts whenever threats change
  useEffect(() => {
    if (!threats || threats.length === 0) return;
    
    const newNodeThreatCounts: Record<string, { high: number, medium: number, low: number, total: number }> = {};
    
    // First, identify all unique node IDs targeted by any threat
    const uniqueNodeIds = new Set<string>();
    threats.forEach(threat => {
      if (threat.target_elements && Array.isArray(threat.target_elements)) {
        threat.target_elements.forEach(nodeId => uniqueNodeIds.add(nodeId));
      }
    });
    ...
    uniqueNodeIds.forEach(nodeId => {
      const nodeCounts = {
        high: threats.filter(t => 
          threatTargetsNode(t, nodeId) && 
          (t.severity || 'MEDIUM').toUpperCase() === 'HIGH'
        ).length,
        medium: threats.filter(t => 
          threatTargetsNode(t, nodeId) && 
          (t.severity || 'MEDIUM').toUpperCase() === 'MEDIUM'
        ).length,
        low: threats.filter(t => 
          threatTargetsNode(t, nodeId) && 
          (t.severity || 'MEDIUM').toUpperCase() === 'LOW'
        ).length,
        total: threats.filter(t => threatTargetsNode(t, nodeId)).length
      };
      
      newNodeThreatCounts[nodeId] = nodeCounts;
    });
    
    setNodeThreatCounts(newNodeThreatCounts);
    
    // Notify parent component about the threat counts and current filter
    if (onNodeThreatCountsChange) {
      onNodeThreatCountsChange(newNodeThreatCounts, severityFilter);
    }
  }, [threats, severityFilter, onNodeThreatCountsChange]);
  ...
  // Add handler for severity filter change
  const handleSeverityFilterChange = (newSeverity: string) => {
    setSeverityFilter(newSeverity);
    
    // Notify parent about the change with current threat counts and new filter
    if (onNodeThreatCountsChange) {
      onNodeThreatCountsChange(nodeThreatCounts, newSeverity);
    }
  };
      ...
      <div className="p-1 border-b border-gray-100 bg-gray-50">
        ...
        {selectedNode && (
          <button 
            ...
            onClick={() => handleSeverityFilterChange(severityFilter === 'NODE' ? 'ALL' : 'NODE')}
          >
          ...
          <div className="space-y-1">
            {filteredThreats.map(threat => (
              <ThreatCard 
                ...
                nodes={nodes} // Pass nodes to each ThreatCard
              />
            ))}
          </div>
        )}
